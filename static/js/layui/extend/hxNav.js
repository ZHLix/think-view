...
;layui.define(['element', 'jquery'], function (exports) {
    ...
    var $ = layui.jquery, element = layui.element, navroot = null, nodes = [], methods = {
        ...
        getData: function (data) {
            internal.getData(data.id, $(nodes[data.id]).find('dl.layui-nav-child'), data.level)
        }
    }, internal = {
        init: function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    $.each(res.data, function (index, val) {
                         ...
                         */
                        a.on('click', function () {
                            if (val.hasChildren && (!val.isExpended) && !$(sub).find('dl.layui-nav-child> dd').length) {
                                internal.getData(val.id, dl, level + 1);
                                val.isExpended = true;
                            }
                            opts.onSelect(val);
